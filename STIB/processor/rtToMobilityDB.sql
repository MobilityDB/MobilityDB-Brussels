...
AS $$
    ...
    vals AS (
        SELECT asMVTGeom(stops(atTime(trip,tstzspan('[' 
		|| getTimestamp(endInstant(trip)) - INTERVAL '5 minutes' || ','
		|| getTimestamp(endInstant(trip)) 
		||']')),
		100,'5 minutes')) AS geom_times FROM stib_trips WHERE tempsubtype(trip) != 'Instant'
    ),
    mvtgeom AS (
        SELECT (geom_times)
        FROM vals
...
/*
with test as (
	select  stops(atTime(trip,tstzspan('[' 
		|| getTimestamp(endInstant(trip)) - INTERVAL '10 minutes' || ','
		|| getTimestamp(endInstant(trip)) 
		||']')),
		100,'5 minutes') from stib_trips where tempsubtype(trip) != 'Instant'
)

select valueset(stops),timespan(stops) from test where stops != null;
*/



--SELECT EXTRACT(EPOCH FROM TIMESTAMPTZ (MIN(tmin(trip::stbox))))::int, EXTRACT(EPOCH FROM TIMESTAMPTZ (MAX(tmax(trip::stbox))))::int from stib_trips;

--SELECT atTime(trip,tstzspan '[2023-12-26 18:49:41+01, 2023-12-26 18:51:41+01)') from stib_trips;